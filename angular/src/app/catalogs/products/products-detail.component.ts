...
import { ProductType, productTypeOptions } from '@proxy/tedu-ecommance/products';
import { NotificationService } from '../../shared/services/notification.service';
import { DomSanitizer } from '@angular/platform-browser';
import { ProductsDto, ProductsInListDto } from '@proxy/catalogs/products/models';
import { ProductsService } from '@proxy/catalogs/products/products.service';
import { ProductsCategoriesService } from '@proxy/catalogs/product-categories/products-categories.service';
import { ManufacturersService } from '@proxy/catalogs/manufacturers/manufacturers.service';
import { ProductCategoryInListDto } from '@proxy/catalogs/product-categories/models';
import { ManufacturerInListDto } from '@proxy/catalogs/manufacturers/models';
...
getNewSuggestionCode(){
  this.productService.getsuggestNumber()
  .pipe(takeUntil(this.ngUnsubscribe))
  .subscribe({
    next:(res:string)=>{
...
loadProductType(){
  ...
  loadFormDetail(id:string){
    ...
    .subscribe({
      next: (response: ProductsDto) =>{
        ...
        this.LoadThumnail(this.selectedEntity.thumnailPicture);
        this.buildForm();
        this.toggleBlockUI(false)
      },
      error(err){
  ...
  private buildForm() {
    this.form = this.fb.group({
      ...
      thumnailPictureName:new FormControl(this.selectedEntity.description || null),
      thumnailPictureContent:new FormControl(null),
    });
  }

  LoadThumnail(fileName:string){
    this.productService.getThumnailImage(fileName)
    .pipe(takeUntil(this.ngUnsubscribe))
    .subscribe({
      next:(response:string) =>{
        var fileExt = this.selectedEntity.thumnailPicture?.split('.').pop();
        this.thumbnailImage = this.sanitizer.bypassSecurityTrustResourceUrl(`data:image/${fileExt};base64,${response}`);
      }
    })
  }
  ...
  onFileChange(event){
    ...
    if(event.target.files && event.target.files.length){
      ...
      reader.onload = () =>{
        this.form.patchValue({
          thumnailPictureName: file.name,
          thumnailPictureContent:reader.result
        });
