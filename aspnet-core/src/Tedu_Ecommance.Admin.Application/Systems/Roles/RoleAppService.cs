...
namespace Tedu_Ecommance.Admin.Systems.Roles
{
       ...
       CreateUpdateRoleDto>, IRoleAppService
    {
        ...
        public async override Task<RoleDto> CreateAsync(CreateUpdateRoleDto input)
        {
            ...
            if (isNameExisted)
            {
                throw new BusinessException(Tedu_EcommanceDomainErrorCodes.RoleNameAlreadyExits)
                    .WithData("Name", input.Name);
            }
            var role = new IdentityRole(Guid.NewGuid(), input.Name);
            role.ExtraProperties[RoleConst.DescriptionFieldName] = input.Description;
        ...
        public async override Task<RoleDto> UpdateAsync(Guid id, CreateUpdateRoleDto input)
        {
            ...
            var query = await Repository.GetQueryableAsync();
            var isNameExited = query.Any(x => x.Name == input.Name);
        ...
        public virtual async Task<GetPermissionListResultDto> GetPermissionsAsync(string providerName, string providerKey)
        {
            ...
            foreach (var group in groups.Where(group => group.Name.StartsWith("AbpIdentity") || group.Name.StartsWith("Tedu_Ecomance")))
