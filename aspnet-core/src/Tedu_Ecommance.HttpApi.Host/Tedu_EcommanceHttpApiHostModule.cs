...
public class Tedu_EcommanceHttpApiHostModule : AbpModule
{
        ...
        if (hostingEnvironment.IsDevelopment())
        {
            Configure<AbpVirtualFileSystemOptions>(options =>
            {
                ...
                options.FileSets.ReplaceEmbeddedByPhysical<Tedu_EcommanceApplicationContractsModule>(
                    Path.Combine(hostingEnvironment.ContentRootPath,
                        $"..{Path.DirectorySeparatorChar}Tedu_Ecommance.Application.Contracts"));
                options.FileSets.ReplaceEmbeddedByPhysical<Tedu_EcommanceApplicationModule>(
                    Path.Combine(hostingEnvironment.ContentRootPath,
                        $"..{Path.DirectorySeparatorChar}Tedu_Ecommance.Application"));
            });
        }
    }
    ...
    private void ConfigureConventionalControllers()
    {
        ...
        {
            options.ConventionalControllers.Create(typeof(Tedu_EcommanceApplicationModule).Assembly);
        });
    }
    ...
    private void ConfigureAuthentication(ServiceConfigurationContext context, IConfiguration configuration)
    {
        context.Services.AddAuthentication(JwtBearerDefaults.AuthenticationScheme)
            .AddJwtBearer(options =>
            {
                ...
                options.Audience = "Tedu_Ecommance";
            });
    }
    ...
    private static void ConfigureSwaggerServices(ServiceConfigurationContext context, IConfiguration configuration)
    {
        context.Services.AddAbpSwaggerGenWithOAuth(
            ...
            {
                    {"Tedu_Ecommance", "Tedu_Ecommance API"}
            },
            options =>
            {
                options.SwaggerDoc("v1", new OpenApiInfo { Title = "Tedu_Ecommance API", Version = "v1" });
    ...
    {
        var dataProtectionBuilder = context.Services.AddDataProtection().SetApplicationName("Tedu_Ecommance");
        if (!hostingEnvironment.IsDevelopment())
        {
    ...
    public override void OnApplicationInitialization(ApplicationInitializationContext context)
    {
        ...
        {
            options.SwaggerEndpoint("/swagger/v1/swagger.json", "Tedu_Ecommance API");
